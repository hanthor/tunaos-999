...
**Key insight**: the build puts the HWE and nvidia layers ON TOP of the DE images, not the other way around. `gnome-hwe` = `yellowfin:gnome` + HWE kernel. The DE is never duplicated.
...
5. **Rechunk** — chunkah makes layers that suit ostree, for delta updates
...
- **CI / Local Gate Execution**:
  ...
  ```
- **Declarative Verification**: `tests/corral/verify.yaml` holds the one YAML scenario, in the Lima style, that configures the boot check.
- **Local Parity**:
  ```bash
...
- SHA pins for the GitHub Actions
- Git submodules
